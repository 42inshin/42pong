...
export class GameGateway {
  ...
  async statusHandler(user: User, type: UserStatusType) {
    await this.userService.updateStatus(user, type, this.loginUserGateway);
  }
  ...
  async handleDisconnect(@ConnectedSocket() client) {
    ...
    if (roomNum != undefined) {
      ...
      let userLeft = "";
      let userright = "";
	  await this.userService.updateStatus(user, UserStatusType.ONLINE, this.loginUserGateway);
  ...
  async rankBattle(@ConnectedSocket() client) {
    ...
    /**status 변경 */
    await this.userService.updateStatus(user, UserStatusType.GAME, this.loginUserGateway);
    /**게임 채팅 참여 유저 목록 */
